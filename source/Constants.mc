...
module Constants {
    ...
    const ART_GL_TRACK_X = 0.0516f;  const ART_GL_TRACK_Y = 0.1645f;
    const ART_GL_FILL_X  = 0.0246f;  const ART_GL_FILL_Y  = 0.1444f;
    const ART_GR_TRACK_X = 0.7645f;  const ART_GR_TRACK_Y = 0.1645f;
    const ART_GR_FILL_X  = 0.7979f;  const ART_GR_FILL_Y  = 0.1449f;
    const ART_FR_TRACK_X = 0.3506f;  const ART_FR_TRACK_Y = 0.6206f;
    const ART_FR_FILL_X  = 0.3215f;  const ART_FR_FILL_Y  = 0.5915f;
    const GAUGE_CY_FRAC  = 0.5f;     // gauges centred on the watch centre (arcs rotated to read level)
    ...
    const GAUGE_BAND_MIDR_FRAC = 0.463f;
    const GAUGE_BAND_HW_FRAC    = 0.017f;   // half clip-box size (fraction of screenW)
    const GL_ANG_0 = 228.6f;  const GL_ANG_100 = 131.6f;   // left gauge: fill CW (decreasing)
    const GR_ANG_0 = 311.3f;  const GR_ANG_100 = 48.4f;    // right gauge: fill CCW (increasing)
